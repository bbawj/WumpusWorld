...
class World:
    ...
    def initialise_world(self, agent_original_position, agent_original_orientation, wumpus_positions, coin_positions, portal_positions, wall_positions):
        ...
        self.world[self.agent.position[1]][self.agent.position[0]].senses['confounded'] = False
    ...
    def update_agent(self, action):
        ...
        if (new_x, new_y) in self.portals:
            ...
            self.world[new_y][new_x].senses['confounded'] = False
    ...
    def display_relative_world(self):
        ...
        for absolute_y in range(len(world)):
            for absolute_x in range(len(world[absolute_y])):
                ...
                relative_y = int(len(world)/2)-absolute_y
                relative_x = -(int(len(world[absolute_y])/2)-absolute_x)
                if len(list(self.db.query(f"visited({relative_x}, {relative_y})"))) == 0: world[absolute_y][absolute_x].is_visited = False
                else: world[absolute_y][absolute_x].is_visited = True

                if len(list(self.db.query(f"safe({relative_x}, {relative_y})"))) == 0: world[absolute_y][absolute_x].is_safe = False
                else: world[absolute_y][absolute_x].is_safe = True
...
if __name__ == "__main__":
    ...
    w = World(agent_original_position=(1, 4), agent_original_orientation='north', wumpus_positions=[(3,4)], coin_positions=[(3,5)], wall_positions=[ (2, 4), (2, 6),  (4, 6), (4, 3)], portal_positions=[(1,1), (6,6)], world_size=(8,8))


    file_name = "StopMug-testPrintout-Self-Friend.txt"
    ###############################
    ...
    while True:
        ...
        next_moves = w.get_next_move()[0]
        next_moves = next_moves[1:-1]
        next_moves = next_moves.split(",")
